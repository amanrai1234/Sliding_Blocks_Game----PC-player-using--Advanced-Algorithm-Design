...
def heuristicks(state):
    heuristic = 0
    for i in range(3):
        for j in range(3):
            heuristic += abs(state[i][j]//3 - i) + abs(state[i][j]%3 - j)
    return heuristic

    
class Node:
    def __init__(self, current, parent, p_action, steps, evaluation):
        self.current=current
        self.parent=parent
        self.p_action = p_action
        self.steps = steps
        self.evaluation = evaluation + steps
...
def astar(init_state):
    queue = []
    initial_node=Node(state_to_string(init_state), None, None, 0, heuristicks(init_state))
    ar.heappush(queue, initial_node)
    visited={}
    visited.update({initial_node.current: initial_node})
    
    while True:
        curnode=ar.heappop(queue)
        current=string_to_state(curnode.current)
        if isgoal(current):
            break
        for check_list in actions(current):
            newstate_arr = swap(current, check_list)
            newstate=state_to_string(newstate_arr)
            if not(newstate in visited):
                node=Node(newstate, curnode, check_list, curnode.steps+1.5, heuristicks(newstate_arr))
                ar.heappush(queue, node)
